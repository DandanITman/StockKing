...
import { alphaVantageService, type StockPrice, type StockHistoryItem } from './alphaVantageService.js';
...
class StockDataService {
  ...
  async updateStockPrice(symbol: string): Promise<StockPrice | null> {
    try {
      ...
      await pool.query(
        ...
        [currentPrice.symbol, currentPrice.price, currentPrice.volume, currentPrice.timestamp]
      );
  ...
  async getPriceHistory(symbol: string, days: number = 30): Promise<PriceHistoryEntry[]> {
    try {
  ...
  async populateHistoricalData(symbol: string, days: number = 30): Promise<boolean> {
    try {
      ...
      for (const item of historicalData) {
        await pool.query(
          ...
          [symbol.toUpperCase(), item.close, item.volume, item.date]
        );
      }
  ...
  async searchStock(query: string): Promise<Array<{symbol: string, name: string}>> {
